...
# B. Provide a dictionary with the count of Oscar wins for each director 
# in the winners list.
...
# We are asking for a list because there could be more than 1 director tied for 
# the most Oscar wins
...
# First we need to create a dictionary to hold the number of times someone emerged winner
win_count_dict = {}
for year, win in winners.items():
    for director in win:
        if director not in win_count_dict:
...
# next we create a variable that will be updated with the name of the person with the highest value for wins
highest_win = 0

# Next, we use a for loop to check fo the highest number of wins
# We set that value as the new highest_win value
# We use the .clear method to remove any key that is lower than the value of the new key and add the new
# key to the most_win_director list
